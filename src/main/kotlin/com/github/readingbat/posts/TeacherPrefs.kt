  ...
  suspend fun PipelineCall.teacherPrefs(content: ReadingBatContent, redis: Jedis): String {
    ...
    return if (user == null || principal == null) {
      ...
      when (val action = parameters[USER_PREFS_ACTION] ?: "") {
        ...
        UPDATE_ACTIVE_CLASS -> updateActiveClass(content, redis, user, forParameter(parameters, CLASSES_CHOICE))
        DELETE_CLASS -> deleteClass(content, redis, user, forParameter(parameters, CLASS_CODE))
