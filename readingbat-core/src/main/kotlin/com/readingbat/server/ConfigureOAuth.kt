...
/**
 * OAuth provider configuration for GitHub and Google authentication.
 *
 * Registers OAuth2 providers with the Ktor Authentication plugin using client ID and
 * secret from HOCON properties. Each provider is configured with its authorize/token URLs,
 * required scopes, and a fallback handler that redirects to the homepage on failure.
 * Providers are silently skipped if credentials are not configured.
 */
internal object ConfigureOAuth {
  ...
  /** Registers the GitHub OAuth2 provider with `user:email` scope. No-ops if credentials are missing. */
  ...
  /** Registers the Google OAuth2 provider with `openid`, `profile`, and `email` scopes. No-ops if credentials are missing. */
