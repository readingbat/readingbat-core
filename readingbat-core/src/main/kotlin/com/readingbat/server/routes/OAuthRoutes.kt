...
/**
 * Registers OAuth login and callback routes for GitHub and Google.
 *
 * Login endpoints redirect to the provider's authorization page. Callback endpoints
 * exchange the authorization code for an access token, fetch the user profile from
 * the provider API, and either link to an existing account or create a new one via
 * [findOrCreateOAuthUser]. The authenticated user principal is stored in the session
 * cookie, and the user is redirected to the saved return URL.
 */
