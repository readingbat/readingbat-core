...
import com.readingbat.common.AuthRoutes.COOKIES
...
import com.readingbat.common.Endpoints.THREAD_DUMP
...
/**
 * Administrative routes for server diagnostics and session management.
 *
 * Provides endpoints for health checks (ping/pong), thread dumps, cookie inspection,
 * and cookie/session clearing. All diagnostic endpoints (except ping) require admin
 * authentication. Also contains [assignBrowserSession] which creates a new browser
 * session cookie on the first request from an untracked client.
 */
object AdminRoutes {
  ...
  /** Creates and assigns a new [BrowserSession] cookie if one is not already present, and triggers geolocation lookup. */
