...
/**
 * WebSocket endpoint for the class summary view.
 *
 * Streams per-student, per-challenge result data (correct/incorrect/unanswered for each
 * invocation) along with incorrect attempt counts and like/dislike indicators. This
 * powers the teacher's class summary grid showing all students' progress across all
...
internal object ClassSummaryWs {
  ...
  fun Routing.classSummaryWsEndpoint(metrics: Metrics, contentSrc: () -> ReadingBatContent) {
    webSocket("$WS_ROOT$CLASS_SUMMARY_ENDPOINT/{$LANGUAGE_NAME}/{$GROUP_NAME}/{$CLASS_CODE}") {
      try {
        ...
        rejectInvalidWsRequest("/websocket_class_summary", e)
      } finally {
  ...
  /** JSON payload for one student's results on one challenge, sent over the class summary WebSocket. */
  @Suppress("unused")
  @Serializable
