...
import com.readingbat.server.ws.WsCommon.GROUP_NAME
...
/**
 * WebSocket endpoint for challenge group statistics.
 *
 * Computes per-challenge aggregate statistics for a class -- including number of students
 * who attempted at least one invocation, number with all correct, average correct count,
 * total incorrect attempts, and like/dislike counts -- and sends them to the connected
 * teacher as JSON frames.
 */
internal object ChallengeGroupWs {
  ...
  fun Routing.challengeGroupWsEndpoint(metrics: Metrics, contentSrc: () -> ReadingBatContent) {
    webSocket("$WS_ROOT$CHALLENGE_GROUP_ENDPOINT/{$LANGUAGE_NAME}/{$GROUP_NAME}/{$CLASS_CODE}") {
      try {
        ...
        rejectInvalidWsRequest("/websocket_class_statistics", e)
      } finally {
  ...
  /** JSON payload containing aggregate statistics for a single challenge in a group. */
  @Serializable
