...
/** Application-wide constants including session keys, color codes, query parameters, and answer states. */
object Constants {
...
/** HOCON property path segments used to construct [Property] names. */
...
/** URL endpoint constants for all HTTP routes and WebSocket paths in the application. */
object Endpoints {
  const val ROOT = "/"
  ...
  const val PRIVACY_POLICY_ENDPOINT = "/privacy-policy"
  const val TOS_ENDPOINT = "/tos"
  ...
  const val OAUTH_LOGIN_ENDPOINT = "$OAUTH_PREFIX/login"
  const val OAUTH_LOGIN_GITHUB_ENDPOINT = "$OAUTH_LOGIN_ENDPOINT/github"
  const val OAUTH_LOGIN_GOOGLE_ENDPOINT = "$OAUTH_LOGIN_ENDPOINT/google"
  const val OAUTH_CALLBACK_GITHUB_ENDPOINT = "$OAUTH_PREFIX/callback/github"
  const val OAUTH_CALLBACK_GOOGLE_ENDPOINT = "$OAUTH_PREFIX/callback/google"
...
/** File names for static image assets (icons, buttons) served from the static content directory. */
...
/** HTML element IDs used in challenge pages for JavaScript interactivity. */
...
/** HTML form field names and action button labels used in user preferences, admin, and class management forms. */
...
/** Named identifiers for Ktor authentication configurations (session, cookie, and OAuth providers). */
...
/** Route paths for authentication-related endpoints. */
