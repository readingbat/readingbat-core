 ...
 *
 * ## Local mode (default):
 *   ./gradlew :readingbat-core:test --tests "com.readingbat.PlaywrightEndpointTest"
 *   Starts an embedded Ktor server with test content on a random port.
 *
 * ## Remote/production mode:
 *   TEST_BASE_URL=https://readingbat.com ./gradlew :readingbat-core:test --tests "com.readingbat.PlaywrightEndpointTest"
...
class PlaywrightEndpointTest : StringSpec() {
  ...
  init {
    ...
    "Navigate to test challenge group page".config(enabledIf = { isLocal }) {
      browser.newPage().use { page ->
        page.navigate("$baseUrl/content/python/${TestData.GROUP_NAME}")
        page.content().apply {
    ...
    "Challenge page displays code and input fields".config(enabledIf = { isLocal }) {
      browser.newPage().use { page ->
        page.navigate("$baseUrl/content/python/${TestData.GROUP_NAME}/float_test")
    ...
    "Navigate between challenges via links".config(enabledIf = { isLocal }) {
      browser.newPage().use { page ->
        ...
        val groupLink = page.locator("a:has-text('${TestData.GROUP_NAME}')")
